...
class Booking {
  ...
  checkAvailability(tableId) {
    ...
    for (
      let hourBlock = thisBooking.hour;
      hourBlock < closeHour;
      hourBlock += 0.5
    ) {
      if (
        typeof thisBooking.booked[thisBooking.date] === 'undefined' ||
        typeof thisBooking.booked[thisBooking.date][hourBlock] ===
          'undefined' ||
  ...
  sendOrder() {
    ...
    for (let starter of thisBooking.dom.starters) {
      if (starter.checked === true) {
        ...
        booking.starters.push(starterValue);
      }
    }
    console.log(thisBooking.dom.starters);

    const bookedTable = thisBooking.dom.wrapper.querySelector(
      select.booking.tables + '[data-table="' + thisBooking.clickedTable + '"]'
    );
  ...
  render(bookingWidgetWrapper) {
    ...
    thisBooking.dom.peopleAmount = bookingWidgetWrapper.querySelector(
      select.booking.peopleAmount
    );
    thisBooking.dom.hoursAmount = bookingWidgetWrapper.querySelector(
      select.booking.hoursAmount
    );
    thisBooking.dom.datePicker = bookingWidgetWrapper.querySelector(
      select.widgets.datePicker.wrapper
    );
    thisBooking.dom.hourPicker = bookingWidgetWrapper.querySelector(
      select.widgets.hourPicker.wrapper
    );
    thisBooking.dom.tables = bookingWidgetWrapper.querySelectorAll(
      select.booking.tables
    );
    thisBooking.dom.starters = bookingWidgetWrapper.querySelectorAll(
      select.booking.starters
    );
    thisBooking.dom.phone = bookingWidgetWrapper.querySelector(
      select.booking.phone
    );
    thisBooking.dom.address = bookingWidgetWrapper.querySelector(
      select.booking.address
    );
    thisBooking.dom.form = bookingWidgetWrapper.querySelector(
      select.booking.form
    );
  }
  initWidgets() {
